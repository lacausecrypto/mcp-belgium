...
async function main() {
  ...
  const port = getPort("PORT_BEST_ADDRESS", 3012);
  ...
  app.get("/health", (_req, res) => {
    res.json({ status: "ok", server: "mcp-best-address", timestamp: new Date().toISOString() });
  });

  app.listen(port, "0.0.0.0", () => {
    logger.info(`MCP server mcp-best-address listening on http://0.0.0.0:${port}/mcp`);
  });
}
